...
def load_policies() -> dict[str, dict[str, Any]]:
    ...
    if payload.get("schema_version") != "sixbench-student-assignments-v2":
    ...
    if not isinstance(cases, dict) or tuple(sorted(cases)) != CASE_NUMBERS:
        raise OperatorError("assignment policy must contain exactly Cases 005/007/009")
    return cases


def validate_release_generation(policy: dict[str, Any], release_id: str) -> None:
    minimum = policy.get("minimum_release_sequence")
    if not isinstance(minimum, int) or minimum < 1:
        raise OperatorError("assignment policy has an invalid minimum release sequence")
    match = re.fullmatch(r"benchmark-v3-pilot-rc([0-9]+)", release_id)
    if match is None:
        raise OperatorError("release id does not follow benchmark-v3-pilot-rcN")
    observed = int(match.group(1))
    if observed < minimum:
        raise OperatorError(
            f"release {release_id} is superseded; Case requires rc{minimum} or newer"
        )
...
def build_context(
    ...
    validate_release_generation(policy, release_id)
        ...
        json.dumps(
            {
                ...
                "track": policy["track"],
                "assignment_state": policy["assignment_state"],
                "minimum_release_sequence": policy["minimum_release_sequence"],
