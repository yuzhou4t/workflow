# HypoWeaver-Qwen 下一步接手与优化路线（历史快照）

> **注意：本文是 2026-07-16 的历史路线快照，不是当前待办清单。** 下文保留当时的问题判断、里程碑和验收思路，便于追溯设计演进；当前运行时事实以 `backend/src/hypoweaver/definition.py`、严格 Schema、测试和主 [`README.md`](README.md) 为准。
>
> 此后已经落地：企业面板/政策 DID Test DAG、代码拥有的 Evidence Registry、无 LLM 的确定性 Claim Gate、Manuscript IR v1，以及 common-executor 的 H2 结果前停止与哈希绑定密封结果恢复。仍未完成的能力不能被接口状态掩盖：六系统 `12/12` 只表示适配接口齐全，HypoWeaver 原生流程不能完整交付 Case 010 的 CR/AR 两个结果变量仍是明确能力缺口，Case 004/010 formal 配对批次也仍关闭。
>
> 文档原始基线：2026-07-16
> 适用范围：Group 2 Task 3——从“研究假设 + 中立数据说明 + 数据资产”到真实验证、审计和论文初稿
> 当时目标：把工作流收敛成“反证优先的证据—主张编译器”，而不是继续增加会写意见的 Agent
