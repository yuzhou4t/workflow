...
- 本 Case 是否已经获准执行，以及只允许运行 `hypoweaver` 还是 `baselines` 分组。
...
它回答的是“这个本地工作区准备运行的，是否正是统一冻结的那一版”。当前 RC9 同时记录绝对路径
和 Python 环境，所以不同工作区要各自自动生成。它不是许可证，不是对学生的人工审批，也不代表
数据已经获准外发。路径、文件或 commit 漂移后应重新生成，而不是手改 lock。

## 4. 哈希绑定 authorization receipt

Receipt 是对一次明确数据外发授权的可校验记录。它必须同时绑定：
...
它回答的是“谁批准把哪一版可见内容发给哪个模型服务”，不绑定具体学生或具体电脑。一份同时覆盖
005/007 的项目级 receipt 可以供本轮四个执行任务复用；只有 Case 内容、协议或模型服务发生变化
时才需要重做。公开模板、GitHub 仓库所有者身份或学生自己的同意都不能替代数据控制方确认。
...
| 正式 receipt | 不建议 | 对特定 Case 哈希、协议和模型服务有效，公开发布不能扩大授权范围 |
