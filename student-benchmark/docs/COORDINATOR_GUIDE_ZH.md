...
不要把案例数据放进公开 Git checkout。正式学生包只放本人 Case 的可见输入；hidden reference、
参考结果、term bindings 和评分材料一律留在负责人侧，不随学生包发放。通过批准的私有渠道放入
固定位置后，检查所有仓库 commit、工作树清洁状态、Python 3.11、依赖和面向执行面的合同测试。
四个工作区使用相同运行代码版本；本机路径和 Python 环境只属于自动兼容性检查，不构成新的人工授权。
...
PYTHONPATH=src .venv/bin/python -m sixbench.outbound_authorization \
  --protocol configs/student-execution-protocol.json \
  --provider-id dashscope \
  --provider-base-url https://dashscope.aliyuncs.com/compatible-mode/v1 \
  --model qwen3.7-plus \
  --output /absolute/private/path/student-execution-outbound-manifest.json
```

每个 Case 使用自己的学生执行协议和 manifest。Manifest 只是待授权内容清单，默认状态不是授权。
先检查它是否只包含本人 Case、两个可见输入视图和最终文件哈希。
...
PYTHONPATH=src .venv/bin/python -m sixbench.release_freeze \
  --release-id <student-release-id> \
  --protocol configs/student-execution-protocol.json \
  --inventory configs/student-execution-inventory.json \
  --provider-id dashscope \
  --provider-base-url https://dashscope.aliyuncs.com/compatible-mode/v1 \
  --model qwen3.7-plus \
  --output /absolute/private/path/student-execution-release-lock.json
```

学生执行版 release lock 会记录本机绝对路径、Python 环境和 capability report，因此必须在各自
工作区自动生成，不能复制另一台机器的 lock 后手工改路径。它证明“本机确实在运行冻结版本”，
不是对该同学进行新的人工审批。

release lock 会校验完整的共同执行板，所以 HypoWeaver 分包初始化时也会安装共享的
data-to-paper 运行时；这只用于冻结校验，不会把五基线单元分配给该同学。
...
一条项目确认可以同时覆盖四个执行任务，但 Case 005 和 Case 007 因为使用不同的可见内容 manifest，
各生成一份技术性 receipt。两份 receipt 都使用 Harness 定义的
`sixbench-outbound-authorization-receipt-v2` Schema，并让
...
同一 Case 的 HypoWeaver 包和五基线包复用同一份 Case receipt；本机 release lock 仍分别生成。
只有冻结 Case、协议或 provider/endpoint/model 发生变化时，才重新生成 manifest 和 receipt，
不需要每次运行重新批准。
...
- 自动生成本机 release lock 的初始化工具、对应 Case 的 outbound manifest 和项目级正式 receipt；
- 自己负责的 4 单元或 20 单元任务书；
- 私有结果回传路径。

API Key 由各执行同学在自己机器上隐藏输入，负责人不收集。收到两份封存包后，项目负责人按
Case 合并 4 + 20 个单元并进行完整性检查，再在负责人侧使用 hidden reference 评分。结果和
hidden reference 永远不提交到 public GitHub。
